...
$results = mysql_unbuffered_query($query);
while($row = mysql_fetch_array($results)){
if($row[0] > 7){ $sum = $sum + 8;
}elseif($row[0] > 5){ $sum = $sum + 6.5;
}elseif($row[0] > 4){ $sum = $sum + 5;
}elseif($row[0] > 2){ $sum = $sum + 3.5;
}elseif($row[0] > 1){ $sum = $sum + 2;
}
}
...
// Create the graph
$graph = new Graph(160,160,$cimgn);
$graph->img->SetAntiAliasing();
//$graph->SetFrame(false);
$graph->SetMargin(30,20,55,30);
