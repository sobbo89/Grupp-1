...
//name the cache img
$results = mysql_query("SELECT f1,f2,f3,f4,f5,f6,f7,f8,f9 FROM chefsutvtabell WHERE GruppID=".$_GET['GruppID']) or die(mysql_error());
while($row = mysql_fetch_array($results)){
for ($val = 1; $val <= 8; $val++){
if(!($data[$val])) $data[$val] = 0;
if(!($data[$val+9])) $data[$val+9] = 0;
if(!($data[$val+18])) $data[$val+18] = 0;
if(!($data[$val+27])) $data[$val+27] = 0;
if(!($data[$val+36])) $data[$val+36] = 0;
if(!($data[$val+45])) $data[$val+45] = 0;
if(!($data[$val+54])) $data[$val+54] = 0;
if(!($data[$val+63])) $data[$val+63] = 0;
if(!($data[$val+72])) $data[$val+72] = 0;
if(round($row[0]) == $val) $data[$val]++;
if(round($row[1]) == $val) $data[$val+9]++;
if(round($row[2]) == $val) $data[$val+18]++;
if(round($row[3]) == $val) $data[$val+27]++;
if(round($row[4]) == $val) $data[$val+36]++;
if(round($row[5]) == $val) $data[$val+45]++;
if(round($row[6]) == $val) $data[$val+54]++;
if(round($row[7]) == $val) $data[$val+63]++;
if(round($row[8]) == $val) $data[$val+72]++;
}
}
$cimgn = 'chefsutv_g_'.$_GET['GruppID'].'_'.implode($data).'.png';
unset($data);
...
for ($x = 1; $x <= 9; $x++){
// Data
$results = mysql_query("SELECT f".$x." FROM chefsutvtabell WHERE GruppID=".$_GET['GruppID']) or die(mysql_error());
// Labels
$fr = mysql_fetch_array(mysql_query("SELECT f".$x." FROM chefsutvfragor WHERE ID=1")) or die(mysql_error());
while($row = mysql_fetch_array($results)){
...
include ("../../jpgraph22/src/jpgraph.php");
include ("../../jpgraph22/src/jpgraph_bar.php");
